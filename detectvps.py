# code revised from: https://github.com/Zhenye-Na/computer-vision-uiuc/blob/0620dee94b7f30bce1f82e3e36cad8456e8fcb43/assignments/assignment3/part3/rectification.py#L17
...
def ransac_vanishing_point(edgelets, num_ransac_iter=2000, threshold_inlier=5):
    ...
    for ransac_iter in range(num_ransac_iter):
        ...
        # ind1 = np.random.choice(first_index_space,size=int(len(first_index_space) > 0))
        # np.random.choice(a, size=int(len(a) > 0))
...
def rectify_image(fig_savepath,name,image, clip_factor=6, algorithm='independent', 
                  reestimate=True):
    ...
    if algorithm == 'independent':
        ...
        if reestimate:
            ...
            # visualization of vp1
            vis_model(image, vp1, show=True)
            save_path = fig_savepath + '//' + name[:-4] + '_vp1.jpg'
        ...
        if reestimate:
            ...
            save_path = fig_savepath + '//' + name[:-4] + '_vp2.jpg'
            plt.savefig(save_path)
            data.append (vp2)
            print(data)
        ...
        print("first estimated vanishing point3: ", vp3)
        if reestimate:
            vp3 = reestimate_model(vp3, edgelets3, 5)
            vis_model(image, vp3, show=True)
            print("after restimated vp3:", vp3)
            save_path = fig_savepath + '//' + name[:-4] + '_vp3.jpg'
            plt.savefig(save_path)
            data.append (vp3)
            print(data)
...
# --------------------对文件夹中的图片的灭点位置分析并保存表格------------

def vpdetect (fig_savepath,name, imagepath):
    #image = io.imread(imagepath,plugin='matplotlib')
    image = cv2.imread('imagepath')
    height, width = image.shape[:2]
    rectify_image(fig_savepath,name,imagepath, 4, algorithm='independent')
    df = pd.DataFrame(data)
    df['width'] = width
    df['height'] =height
    df['x_distance'] = abs(df[0] / df['width'] - 0.5)
    df['y_distance'] = abs(df[0] / df['height'] - 0.5)
    df['distance'] = ((df['x_distance'] ** 2) + (df['y_distance'] ** 2)) ** 0.5
    df
